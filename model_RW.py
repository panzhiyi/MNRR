...
class DifNet(nn.Module):
    def __init__(self, num_classes, layers ,BatchNorm=nn.BatchNorm2d, shrink_factor=2):
        ...
        self.conv51 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, padding=1, bias=False),
                                   ...
                                   nn.ReLU())
        '''
        self.conv52 = nn.Sequential(nn.Conv2d(inter_channels + 512, inter_channels, 3, padding=1, bias=False),
                                   BatchNorm(inter_channels),
                                   nn.ReLU())
        self.conv53 = nn.Sequential(nn.Conv2d(inter_channels + 256, inter_channels, 3, padding=1, bias=False),
                                   BatchNorm(inter_channels),
                                   nn.ReLU())
        '''
    ...
    def forward(self, x):
        ...
        pred4 = self.conv51(pred4)
        pred = self.conv6(pred4)
