...
class MyArgumentParser():
    def __init__(self, inference=False):
        ...
        self.parser.add_argument('dataset', metavar='DATASET', help='dataset: VOC2012|cityscapes|SBD')
        ...
        self.parser.add_argument ( 'delta_weight', default=0, type=float, metavar='DELTA',
                                   help='delta weight' )
        self.parser.add_argument ( 'rho_weight', default=0, type=float, metavar='RHO',
                                   help='rho weight' )
        self.parser.add_argument ('sysrand', metavar='System Random', help='system random: Entropy|Var|Gini')
        if inference:
            ...
            self.parser.add_argument('entropyloss_weight', default=0, type=float, metavar='ENTROPYLOSS_WEIGHT',help='entropyloss_weight')
            self.parser.add_argument('balance_weight', default=1, type=float, metavar='BALANCELOSS_WEIGHT',help='balance weight for edge and not')
            self.parser.add_argument('losstype', default='BCE', metavar='LOSS_TYPE',help='MSE or BCE')
    def get_parser(self):
        return self.parser
...
def param_restore_HED(model, param_dict):
    new_params = model.state_dict().copy()
    for i in param_dict:
        i_parts = i.split('.')
        #print(i)
        if i_parts[0]!='features' and i_parts[0]!='classifier':
            new_params[i] = param_dict[i]
    model.load_state_dict(new_params)
    return model
...
def save_checkpoint(state, date, is_best, shfilename, filename='checkpoint.pth.tar'):
    ...
    if not os.path.exists(directory + shfilename):
        ...
        shutil.copyfile('./train_seg_UR.py', directory + 'train_seg_UR.py')
    filename = directory + filename
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, directory + 'model_best.pth.tar')
    #if state['epoch']==50:
    #    shutil.copyfile(filename, directory + '_50.pth.tar')
